...
import pathlib
argv = sys.argv
sys.path.append("/projects/tewhey-lab/mourik/MPRAduo_Hannah/")
...
# A_tag_asso = '/projects/tewhey-lab/deweyh/oligo_align/MPRAduo_LibA_pilot_20191120.1.merged.match.mapped.barcode.ct.parsed'
# P_tag_asso = '/projects/tewhey-lab/deweyh/oligo_align/MPRAduo_LibP_pilot_20191120.1.merged.match.mapped.barcode.ct.parsed'

#create lists of the bar codes for each library
BC_A = pd.read_table(dictA, header=None, index_col=False, usecols=[0,1])
BC_P = pd.read_table(dictP, header=None, index_col=False, usecols=[0,1])
...
with open("%s/%s/perfect_match.txt" % (current_path, out_dir), "w") as perfect_oligo:
    with open("%s/%s/partial_match.fastq" % (current_path, out_dir), "w") as partial_fastq:
        with open("%s/%s/partial_match.txt" % (current_path, out_dir), "w") as partial_oligo:
            with open("%s/%s/single_no_match.fastq" % (current_path, out_dir), "w") as no_match_single:
                with open("%s/%s/duo_no_match.fastq" % (current_path, out_dir), "w") as no_match_duo:
                    with open("%s/%s/oligo_no_match.fastq" % (current_path, out_dir), "w") as no_match_oligo:
