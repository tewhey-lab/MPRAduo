...
with open ("/projects/tewhey-lab/mourik/MPRAduo_Hannah/output/%s/matched_all.txt" % out_dir, "w") as matched_all_oligo:
                        ...
                        if len(seq_only[fill_start:]) >= 110:
                            ...
                            if lib_A_index > 0 and lib_AiP_index > 0:
                                ...
                                if g in libA_dict and k in libP_dict:
                                    matched_all_oligo.write("%s^%s\t%s^%s\n" % (g,k,libA_dict[g],libP_dict[k]))
                            ...
                            if lib_P_index > 0 and lib_PiA_index > 0:
                                g = seq_only[lib_P_index+6:lib_P_index+26]
                                k = seq_only_PiA[lib_PiA_index+6:lib_PiA_index+16]
...
with open ("/projects/tewhey-lab/mourik/MPRAduo_Hannah/output/%s/statistics.txt"  % out_dir, "w") as run_stats:
    ...
    run_stats.write("Percent Matched with No Oligos: %f" % float(no_oligo_match/((count_duo-count_duo_no_match)+(count_single-count_single_no_match))))
