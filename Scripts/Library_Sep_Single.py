...
with open("%s.match" % out_dir, "w") as match:
    ...
    # 3 steps check for ES, then SE, then assume single if not present in either.
                                    if ES_indic in seq_only[fill_start+71:fill_start+85]:
                                        ...
                                        if lib_E_index in range(38,50) and lib_S_index in range(93,105):
                                            ...
                                            if bc_E in libE_dict and bc_S in libS_dict:
                                                ...
                                                SeqIO.write(record, partial_fastq, "fastq")
                                            else bc_E not in libE_dict and bc_S not in libS_dict:
                                                no_oligo_match += 1
                                                count_duo_ES_no_oligo += 1
                                                SeqIO.write(record, no_match_oligo, "fastq")
                                        elif lib_E_index in range(38,50) and lib_S_index not in range(93,105):
                                            count_duo_no_match_ES += 1
                                            count_duo_ES_no_S += 1
                                        elif lib_E_index not in range(38,50) and lib_S_index in range(93,105):
                                            count_duo_no_match_ES += 1
                                            count_duo_ES_no_E += 1
                                        else:
                                            count_duo_no_match_ES += 1
                                    elif SE_indic in seq_olnly[fill_start+81:fill_start+95]:
                                        count_duo_SE += 1
                                        lib_E_index = seq_only.index(link_E)
                                        lib_S_index = seq_only.index(link_S)
                                        if lib_S_index in range(38,50) and lib_E_index in range(93,105):
                                            k = seq_only[lib_E_index+6:lib_E_index+16]
                                            g = seq_only[lib_S_index+6:lib_S_index+26]
                                            bc_E = seq(k)
                                            bc_S = seq(g)
                                            ...
                                            if bc_E in libE_dict and bc_S in libS_dict:
                                                ...
                                                SeqIO.write(record, no_match_oligo, "fastq")
                                        elif lib_S_index in range(38,50) and lib_E_index not in range(93,105):
                                            count_duo_no_match_SE += 1
                                            count_duo_SE_no_S += 1
                                        elif lib_S_index not in range(38,50) and lib_E_index in range(93,105):
                                            count_duo_no_match_ES += 1
                                            count_duo_SE_no_E += 1
                                        else:
                                            count_duo_no_match_SE += 1
